(() => {
  ...
  function handleClick(event) {
    ...
    if (action === 'track-order') { state.trackingOrder = state.orders.find((order) => String(order.id) === String(target.dataset.orderId)) || state.orders[0]; state.modal = 'tracking'; render(); return; }
